...
public class StudentGeneralSettings extends JPanel {
				...
				if (btnEditSettings.isSelected()) {
					...
					case JOptionPane.OK_OPTION:
						...
						try {
							...
							passtest.setInt(1,Integer.parseInt(Login.usrname));
					...
					if(Login.isfieldsempty(NameField,FathersNameField,MothersNameField,ContactField,EmailField,AddressArea)){
						...
						case JOptionPane.YES_OPTION:
							try {
								String query = "UPDATE student_details SET DoB = ?, phone = ?, address = ?, email = ? WHERE rollno = ?";
								PreparedStatement pst = connection.prepareStatement(query);
								pst.setString(1, dob.getText());
								pst.setString(2, ContactField.getText());
								pst.setString(3, AddressArea.getText());
